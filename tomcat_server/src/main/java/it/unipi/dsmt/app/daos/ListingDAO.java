...
public class ListingDAO {
    ...
    public List<ListingDTO> getListings() throws SQLException {
        ...
        while (set.next()) {
            ListingDTO listing = new ListingDTO(set.getInt("ID"),set.getString("username"),set.getInt("boxIDwinner"), set.getTimestamp("timestamp"),set.getString("pokemonName"),set.getString("primaryType"),
    ...
    public List<ListingDTO> getListingsByUsername(String username) throws SQLException {
        ...
        while (set.next()) {
            ...
            System.out.println();
            ListingDTO listing = new ListingDTO(set.getInt("ID"),username,set.getInt("boxIDwinner"), set.getTimestamp("timestamp"),set.getString("pokemonName"),set.getString("primaryType"),
                    set.getString("secondaryType"),set.getInt("attack"),set.getInt("defense"),set.getString("imageURL"));
            result.add(listing);
    ...
    public boolean setWinner(int listingID, int winnerID) throws SQLException {
        String sqlString = "UPDATE listing SET boxIDwinner = ? WHERE ID = ?";
        PreparedStatement statement = listingConnection.prepareStatement(sqlString);
        statement.setInt(1, listingID);
    ...
    public String insertListing(Listing listing) throws SQLException  {
        try {
            String sqlString = "INSERT INTO listing(boxID, boxIDwinner, timestamp) VALUES (?, ?, ?)";
            PreparedStatement statement = listingConnection.prepareStatement(sqlString);
            statement.setInt(1, listing.getBoxID());
            statement.setInt(2, listing.getWinner());
